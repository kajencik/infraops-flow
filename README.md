...
InfraOps Flow is an operations notebook for the operator who started with remembered IPs, half-finished migrations, old forum tabs, and a strong belief that the documentation cleanup would happen later.
...
InfraOps Flow is the version after that operator got tired of operating by folklore, noise, and half-visible traces.
...
1. `data/knowledge-graph-top-dashboard.example.json` — top projects and open items in one small read
2. `data/knowledge-graph-overview.example.json` — domain map and project pointers
3. `inventory/devices.example.json`
4. `inventory/service-topology.example.json`
5. `data/recurring-checks.json`
6. `notes/_example-session.md`
7. `notes/projects/_example-project.md`
8. `scripts/host-health.ps1`
