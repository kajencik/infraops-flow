...
In the sample environment, the notes tell the actual transition story: from a setup that mostly worked through habit and memory to one that can survive interruption, handoff, and a bad Tuesday evening without losing the plot.
...
The notes are where weak intuitions become known state.
